...
# The board adoption / signatures page is redrawn by the formatter (only on the
# signed variant), so it must not be imported into the body.
BOARD_MARK = ("adopted by the Board of Directors", "As adopted by the Board")
...
def build_docx(pdf, out):
    ...
    # Drop the version/owner-approver card and the board adoption/signatures
    # page (both regenerated by the formatter) and everything after them. The
    # board block can be a plain block or a two-column (signature names) block.
    def _marked(k, p):
        if k == "block":
            return (any(m in p[0] for m in VCARD_MARK) or any(m in p[0] for m in BOARD_MARK)
                    or bool(re.match(r"\s*Owner\s*:", p[0])))
        if k == "table":
            return _is_vcard_table(p)
        if k == "columns":
            flat = " ".join(cs for col in p for (cs, *_rest) in col)
            return any(m in flat for m in VCARD_MARK) or any(m in flat for m in BOARD_MARK)
        return False
    cut = len(items)
